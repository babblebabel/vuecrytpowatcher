    ...
    export default {
      ...
      data() {
        return {
          ...
          previousCurrency: {
            yesterday: { BTC:1000,LTC: 2000, ETH: 3000}, twoDays: {}, threeDays: {}, fourDays: {}, fiveDays: {}
          }
        }
      },
      methods: {
        _fetchDataFor : (key, daysAgo)=> {
         //--Targeting the understanding of scope for this.axios&this.moment
          self = this
          var date = moment().subtract(daysAgo,'days').unix()
          console.log('Beginning of the dates')
          console.log(date)
          console.log('End of the dates')
          ...
          let fetchBTC = fetch('BTC',date)
          let fetchETH = fetch('ETH',date)
          let fetchLTC = fetch('LTC',date)

          axios.all([fetchBTC, fetchETH,fetchLTC])
          .then(axios.spread((BTC,ETH,LTC)=> {
            self.previousCurrency[key]={
      ...
      created() {
        if(!navigator.onLine){
          ...
          this._fetchDataForToday()
           let pusher = new Pusher('78ea69ec5b57dec0b1af', {
      cluster: 'ap2',
      encrypted: true
    });
